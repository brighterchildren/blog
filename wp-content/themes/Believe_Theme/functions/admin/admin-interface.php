	...
	foreach ($options as $value) {
		...
		switch ($value['type']) {
			...
			case "heading":
				...
				$menu .= '<li class="tab_' . strtolower(str_replace(" ", "_", $value['name'])) . '"><a title="' . $value['name'] . '" href="#' . $jquery_click_hook . '">' . $value['name'] . '</a></li>';
		...
		if ($value['type'] != "heading") {
			...
			if (!empty($tooltip)) {
				$output .= '
			<a rel="tooltip" class="tooltip-icon" id="' . $tooltip . '"></a>';
			}
